...
# 5. Unified research collector: BTC+XAG+XAUT L2 + XAUT options surface in
#    ONE process (one-per-symbol OOM-killed the container on 2026-08-04 —
#    each tsx runtime costs ~150-250MB; the shared-process marginal cost per
#    extra market is just its book). Non-fatal.
ORDERFLOW_SYMBOLS=BTCUSDT,XAGUSDT,XAUTUSDT NODE_OPTIONS=--max-old-space-size=384 \
  npx tsx scripts/collect-btc-orderflow.ts --with-xaut-options &
FLOW_PID=$!
...
echo "  crypto=$CRYPTO_PID gold=$GOLD_PID metals=$METALS_PID governor=$GOV_PID collector=$FLOW_PID ui=$UI_PID letf=$LETF_PID"

# Core processes whose death should restart the whole container.
CORE_PIDS="$CRYPTO_PID $GOLD_PID $METALS_PID $GOV_PID"

ALL_PIDS="$CRYPTO_PID $GOLD_PID $METALS_PID $GOV_PID $FLOW_PID $UI_PID $LETF_PID"
