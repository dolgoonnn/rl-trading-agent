...
✅ **Microstructure research now ACCUMULATING (was blocked).** The collector had STALLED on 2026-06-12 (only 2 partial days). 2026-06-14: RESTARTED it (running, connected to Bybit WS, writing fresh BTCUSDT_2026-06-14.ndjson 1s snapshots) AND added a durable PM2 entry `orderflow-collector` (autorestart, max_restarts 1000). **To keep it alive across reboots, run: `pm2 start ecosystem.config.cjs --only orderflow-collector`.** Once ≥30–60d accrue, the OFI/CVD + liquidation-fade research items unblock and re-enter the queue. NOTE: collector is BTCUSDT-only — add ETH/SOL collectors if a multi-symbol microstructure study is wanted. (The background process I started will collect for this session; PM2 is the durable path.)
