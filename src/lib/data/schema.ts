...
// ============================================================================
// PROPOSAL QUEUE (Layer 4 — bounded autonomy / approval workflow)
// ============================================================================
//
// Each row is a trade idea generated by a strategy ("Layer 3" — the confluence
// scorer, F2F, etc). The user approves/rejects via the queue UI; decisions are
// persisted with a required reason so the audit log can compute counterfactuals
// later ("you rejected 12 today; 7 would have won").
//
// Lifecycle: pending → (approved | rejected | snoozed | expired) → executed?
// outcomePnlR is filled later (by a counterfactual job for rejected/snoozed,
// by the live order outcome for approved).
...
export const proposals = sqliteTable('proposals', {
  ...
  regime: text('regime').notNull(),
  // Reasoning bundle (LATTICE-shaped): factor breakdown, top contributors,
  // historical bucket stats, caveats. Stored as JSON for forward compat.
  reasoning: text('reasoning').notNull(),
  caveats: text('caveats').notNull(), // JSON array of strings

  status: text('status').notNull().default('pending'), // pending|approved|rejected|snoozed|expired|executed
  decisionReason: text('decision_reason'), // required when approved/rejected
  decisionAt: integer('decision_at'),
  snoozeUntil: integer('snooze_until'),

  // Outcome tracking (filled async)
  outcomePnlR: real('outcome_pnl_r'), // R-multiple at exit / synthetic exit
