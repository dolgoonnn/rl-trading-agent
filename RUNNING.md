...
> ⚠️ **Residual post-partial divergence (until Phase 2b).** The exchange still holds a 100 % SL+TP (`tpslMode: 'Full'`); the fractional partial-TP is booked only in the in-process shadow (never sent to the venue). So after a partial, the shadow's blended PnL can still differ from the real Bybit balance (e.g. reversal-to-BE: the venue sells 100 % at BE while the shadow already booked the partial profit). Phase 2a fixes the "manages a flat position" problem; **Phase 2b** — moving the fractional partial onto the exchange as a resting reduce-only order (intrabar-vs-close parity change, re-measure Run-20) — closes the remaining gap. Until then, when live with `--exchange-exits`, **reconcile realized PnL against the Bybit account balance**, not the equity snapshots.
